...
fn filter_lines(lines: Vec<String>, filter_keywords: &[&str]) -> Vec<String> {
    lines.into_iter().filter(|line| {
        !filter_keywords.iter().any(|&keyword| line.contains(keyword))
    }).collect()
}
...
fn write_filtered_lines(filename: &str, game: Game) -> io::Result<()> {
    let results_path: std::path::PathBuf = Path::new(RESULTS_DIR).join(filename).with_extension("json");
...
fn construct_game(input: Vec<String>, power_map: &HashMap<String, String>) -> Game {
	// group string based on rules
    let mut i = 0;
	let mut phases: HashMap<String, Phase> = HashMap::new();
	let mut curr_sc_dist: HashMap<String, Vec<String>> = HashMap::new();
	let mut curr_unit_dist: HashMap<String, Vec<String>> = HashMap::new();
		...
		if is_boardcast {
			...
			if line.contains("ORD") {
				...
				if line.contains("SCO") {
					let sc_dist: Vec<String> = sc_dist_re.find_iter(line)
						.map(|m| m.as_str().to_string())
						.collect();
					// map to remove the starting and ending brackets
					let sc_dist: Vec<String> = sc_dist.iter()
					...
					let line: &str = line.trim();
					assert!(!line.contains("NOW"), "NOW should not be in the line: {}", line);
								...
								if c == ')' {
									...
									if s.contains("19") {
										...
										if let Some(units) = curr_unit_dist.get_mut(power) {
											let unit_str: String = unit_type.chars().next().unwrap().to_string() + " " + unit;
											units.push(unit_str);
										}
...
fn main() -> io::Result<()> {
    ...
    for path in paths {
        ...
        if path.is_file() {
			...
			// trim all lines
			lines.iter_mut().for_each(|line: &mut String| *line = line.trim().to_string());
			...
			let mut power_map: HashMap<String, String> = HashMap::new();
			...
			for _ in 0..7 {
				...
				let country_code: &str = re.captures(trimmed)
